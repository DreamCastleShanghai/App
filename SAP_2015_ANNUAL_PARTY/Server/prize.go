...
type UserView struct {
	...
	IsPrized     bool   `gorm:"column:IsPrized"`
}

var gRelease bool = true
var gLocal bool = false

var gPrizeString string
var gGroupString string
...
func main() {
	...
	for i := 1; i < argCnt; i++ {
		if os.Args[i] == "debug" {
			...
			gPrizeString = os.Args[i+1]
		} else if os.Args[i] == "g" {
			gGroupString = os.Args[i+1]
		}
	}
	gPrize, _ := strconv.Atoi(gPrizeString)
	gGroup, _ := strconv.Atoi(gGroupString)

	MyPrint("Release Mode : ", gRelease)

	MyPrint(gGroupString + " win the " + gPrizeString + " prize......")
	...
	if gDB != nil {
		...
		gDB.Raw("SELECT * FROM User WHERE (VoiceVoteId1 = ? OR VoiceVoteId2 = ?) AND IsPrized = false", gGroup, gGroup).Scan(&users)
		totalcnt = len(users)
		MyPrint(gPrizeString+" prize users cnt : ", totalcnt)
		if totalcnt > PRIZE_CNT {
			totalcnt = PRIZE_CNT
		}
		for i := 0; i < totalcnt; {
			r := rand.New(rand.NewSource(time.Now().UnixNano()))
			id := r.Intn(totalcnt)
			if !users[id].IsPrized {
				gDB.Exec("INSERT INTO Winner (UserId, WinnerType) VALUES (?, ?)", users[id].UserId, gPrize)
