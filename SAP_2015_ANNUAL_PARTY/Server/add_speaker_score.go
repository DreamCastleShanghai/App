...
func main() {
	...
	relations := []SpeakerSessionRelation{}

	if gDB != nil {
		gDB.Raw("Select * from Speaker_Session_Relation").Scan(&relations)
		for _, relation := range relations {
			sidInt := strconv.Itoa(relation.SessionId)
			AddUserScore(relation.SpeakerId, SpeakerOfOwnSessionID, "Speaker of Session "+sidInt)
		}
	}
